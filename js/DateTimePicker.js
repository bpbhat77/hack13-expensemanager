...
function timeDateDifference(endDate,startDate) {
...
var startDateTime =getsplitdate(dts);

 var endDateTime = getsplitdate(ets);

 var dayDiff = timeDateDifference(endDateTime,startDateTime) ;
 return dayDiff;
 }
function getsplitdate(dateinusr){
 var y,m,d,h,mi;
//04 January 2013 22:44
var temp = dateinusr.replace(":"," ");
var stringArray = temp.split(" ");
y=stringArray[2];
m=getmonth(stringArray[1]);
d=stringArray[0];
h=stringArray[3];
mi=stringArray[4];
...
function min(t){
    var cd = 24 * 60 * 60 * 1000,
        ...
        m =  Math.round( (t - d * cd - h * ch) / 60000)+h*60;
    return m;
}
